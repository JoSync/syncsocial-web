...
export default function CreatorsPage() {
  ...
  const sl = [
    ...
    {t:"21:00 Analytics", d:"Revenue reporting", x:"$14,200 Generated"}
  ];
              ...
              <div className="flex-1 p-10 bg-slate-50/50 flex flex-col justify-center">
                <div key={curr} className="animate-fade-in space-y-6">
                  <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-violet-100">
                    <p className="text-3xl font-black text-violet-950 italic leading-none mb-2">{sl[curr].t}</p>
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">{sl[curr].d}</p>
                  </div>
                  <div className="bg-violet-950 p-6 rounded-[2rem] shadow-lg flex items-center justify-between">
                    <span className="text-xs font-black text-violet-200 uppercase tracking-widest">{sl[curr].x}</span>

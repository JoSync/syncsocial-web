...
export default function UsersPage() {
  ...
  const pData = c?.p?.users || { s: "Never miss a beat." };

  const sl = [
    {t:"Live Update", d:"Tom is now in the Amazon", x:"GPS Sync: Moving North", i:<Globe/>, c:"bg-emerald-600"},
    {t:"Drop Alert", d:"Excl. Sneaker Release", x:"Access unlocked", i:<ShoppingBag/>, c:"bg-rose-600"},
    {t:"Community", d:"Group Video Call", x:"12 friends in-sync", i:<Video/>, c:"bg-violet-600"},
    {t:"Reminder", d:"Twitch Stream starts", x:"Added to calendar", i:<Clock/>, c:"bg-indigo-600"}
  ];
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-violet-100 overflow-x-hidden relative">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 grid lg:grid-cols-2 gap-20 items-center">
        ...
        <div className="relative"><div className="absolute -inset-10 bg-emerald-500/10 blur-[100px] rounded-full"></div>
          <div className="relative bg-violet-950 rounded-[4rem] p-3 shadow-3xl border-[12px] border-violet-900 overflow-hidden"><div className="bg-white rounded-[3rem] overflow-hidden h-[600px] flex flex-col relative text-left">
              ...
              <div className="flex-1 p-10 bg-slate-50/50 flex flex-col justify-center"><div key={curr} className="animate-fade-in space-y-6"><div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-violet-100 flex items-center gap-6"><div className={`w-16 h-16 ${sl[curr].c} rounded-2xl flex items-center justify-center text-white shadow-xl`}>{sl[curr].i}</div><div className="flex-1"><p className="text-2xl font-black text-violet-950 italic leading-none mb-1">{sl[curr].t}</p><p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{sl[curr].d}</p></div></div><div className="bg-violet-950 p-6 rounded-[2rem] shadow-lg flex items-center justify-between"><span className="text-xs font-black text-violet-200 uppercase tracking-widest">{sl[curr].x}</span><ArrowRight className="text-emerald-400" size={20}/></div></div></div>
            </div></div></div>
      </main>

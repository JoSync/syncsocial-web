  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Oeps! Er ging iets mis. Controleer of je het juiste test-emailadres gebruikt.");
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-indigo-100 overflow-x-hidden relative">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-12 pb-32 text-center flex flex-col items-center relative">
        ...
        <div className="w-full max-w-xl mx-auto mb-32 flex flex-col items-center">
          {!isSubmitted ? (
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row w-full animate-fade-in shadow-[0_25px_50px_-12px_rgba(67,56,202,0.25)] rounded-full overflow-hidden border border-indigo-100 bg-white">
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder={lang === 'CN' ? '电子邮件' : 'Email address'} 
          ...
          <div className="flex items-center gap-3 mt-8">
             <div className="flex -space-x-3">
               {[1,2,3,4].map(i => <div key={i} className="w-9 h-9 rounded-full border-2 border-indigo-50 bg-white shadow-sm flex items-center justify-center text-indigo-300"><Users size={14} /></div>)}
             </div>
             <div className="flex items-center gap-2 text-slate-500">
               <Sparkles size={14} className="text-indigo-400" />
               <p className="text-[10px] font-black uppercase tracking-widest italic">Join 500+ creators in sync</p>
             </div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-[2.5rem] overflow-hidden h-[600px] flex flex-col relative shadow-inner">
              <div className="bg-white border-b border-slate-50 p-6 flex justify-between items-center text-left sticky top-0 z-10">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left leading-tight"><p className="font-black text-indigo-950 text-sm italic">Maxime & Sophie</p><p className="text-[10px] text-emerald-500 font-black uppercase tracking-widest flex items-center gap-1 mt-1"><ShieldCheck size={10} /> {c.m_status}</p></div>
                </div>
                <Globe size={20} className="text-indigo-600 animate-spin-slow" />
              </div>
              <div className="flex-1 relative p-8">
                {currentSlide === 0 && <div className="animate-fade-in text-left">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 font-bold">{c.m_today}</p>
                  <div className="p-6 bg-indigo-50 border border-indigo-100 rounded-3xl shadow-sm flex items-center gap-4">
                    ...
                    <div className="text-left"><p className="font-black text-indigo-950 text-lg leading-none italic">Merch Drop</p><p className="text-[10px] font-bold text-indigo-400 mt-1 uppercase">14:00 GMT</p></div>
                  </div>
                </div>}
                ...
                {currentSlide === 3 && <div className="animate-fade-in flex flex-col items-center justify-center h-full"><BarChart3 size={64} className="text-emerald-500 mb-4 animate-bounce-slow" /><p className="text-2xl font-black text-indigo-950 italic tracking-tighter">AI Analytics</p></div>}
              </div>
      ...
      <section className="py-32 relative z-10">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-4xl lg:text-7xl font-black text-indigo-950 tracking-tighter uppercase text-center mb-20 italic underline decoration-indigo-600 underline-offset-8">{c.fTitle}</h2>
          <div className="grid lg:grid-cols-3 gap-8">
            {[ { i: <ShoppingBag />, t: c.f1, d: c.f1d }, { i: <BarChart3 />, t: c.f2, d: c.f2d }, { i: <ShieldCheck />, t: c.f3, d: c.f3d } ].map((f, i) => (
              <div key={i} className="p-12 rounded-[3.5rem] bg-white border border-slate-100 hover:border-indigo-100 transition-all group relative overflow-hidden text-left">

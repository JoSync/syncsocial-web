...
import { ShoppingBag, Globe, ArrowRight, CheckCircle2, ShieldCheck, BarChart3, Users, Sparkles, ChevronDown } from 'lucide-react';
    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-violet-100 overflow-x-hidden relative">
      ...
      <nav className="flex justify-between items-center p-8 max-w-7xl mx-auto relative z-50">
        ...
        <div className="relative">
          <button onClick={() => setShowLang(!showLang)} className="flex items-center gap-2 bg-white/60 backdrop-blur-xl px-4 py-2 rounded-full border border-slate-200 text-xs font-black text-violet-950 shadow-sm transition hover:bg-white">
            {lang} <ChevronDown size={14} className={`transition-transform ${showLang ? 'rotate-180' : ''}`} />
          </button>
          {showLang && (
            <div className="absolute right-0 mt-2 w-24 bg-white/90 backdrop-blur-2xl rounded-2xl border border-slate-100 shadow-2xl overflow-hidden flex flex-col z-[60]">
              {['EN', 'NL', 'ES', 'CN'].map((l) => (
                <button key={l} onClick={() => { setLang(l); setShowLang(false); }} className={`px-4 py-3 text-xs font-black text-left hover:bg-violet-50 transition ${lang === l ? 'bg-violet-100 text-violet-600' : 'text-slate-600'}`}>{l}</button>
              ))}
            </div>
          )}
        </div>
          ...
          <div className="relative bg-violet-950 rounded-[3.5rem] p-3 shadow-2xl border-[12px] border-violet-900 overflow-hidden ring-1 ring-white/10">
            <div className="bg-white rounded-[2.5rem] overflow-hidden h-[600px] flex flex-col relative shadow-inner text-left">
              <div className="bg-white border-b border-slate-50 p-6 flex justify-between items-center sticky top-0 z-10">
                <div className="flex items-center gap-3"><div className="w-12 h-12 bg-violet-950 rounded-full flex items-center justify-center text-white font-black text-xl border-2 border-white shadow-lg">MS</div><div className="leading-tight"><p className="font-black text-violet-950 text-sm italic">Maxime & Sophie</p><p className="text-[10px] text-emerald-500 font-black uppercase flex items-center gap-1 mt-1"><ShieldCheck size={10} /> {c.ms}</p></div></div><Globe size={20} className="text-violet-600 animate-spin-slow" />
              </div>
              <div className="flex-1 relative p-8">
                {currentSlide === 0 && <div className="animate-fade-in"><p className="text-[10px] font-black text-slate-400 uppercase mb-6">{c.mt}</p><div className="p-6 bg-violet-50 border border-violet-100 rounded-3xl shadow-sm flex items-center gap-4"><div className="w-12 h-12 bg-violet-600 rounded-2xl flex items-center justify-center text-white shadow-lg"><ShoppingBag size={24} /></div><div><p className="font-black text-violet-950 text-lg leading-none italic">Merch Drop</p><p className="text-[10px] font-bold text-violet-400 mt-1 uppercase">14:00 GMT</p></div></div></div>}
                {currentSlide === 1 && <div className="animate-fade-in text-slate-400 uppercase text-[10px] font-black tracking-widest">{c.ml}...</div>}
                {currentSlide === 2 && <div className="animate-fade-in flex flex-col items-center justify-center h-full"><Globe size={48} className="text-violet-400 mb-4" /><p className="font-black text-violet-950 italic">{viewers.toLocaleString()} fans in sync</p></div>}
              </div>
            </div>
          </div>
        </div>

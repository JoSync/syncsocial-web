...
export default function LandingPage() {
  ...
  useEffect(() => {
    const sInt = setInterval(() => setCurrentSlide((p) => (p + 1) % 3), 4000);
      ...
      <section className="max-w-7xl mx-auto px-6 pt-12 pb-32 text-center flex flex-col items-center relative z-10">
        ...
        <div className="relative w-full max-w-3xl mt-24">
          ...
          <div className="relative bg-violet-950 rounded-[3.5rem] p-3 shadow-2xl border-[12px] border-violet-900 overflow-hidden ring-1 ring-white/10">
            <div className="bg-white rounded-[2.5rem] overflow-hidden h-[500px] flex flex-col relative shadow-inner text-left">
              ...
              <div className="flex-1 relative p-8">
                <div className="animate-fade-in"><p className="text-[10px] font-black text-slate-400 uppercase mb-6 italic">{c.mt}</p>
                  <div className="p-8 bg-violet-50 border border-violet-100 rounded-[2.5rem] shadow-sm flex items-center gap-6 mb-8">
                    <div className="w-16 h-16 bg-violet-600 rounded-2xl flex items-center justify-center text-white shadow-lg">{c.slides[currentSlide].i}</div>
                    <div><p className="font-black text-violet-950 text-xl leading-none italic">{c.slides[currentSlide].t}</p><p className="text-[11px] font-bold text-violet-400 mt-2 uppercase">{c.slides[currentSlide].d}</p></div>
                  </div>
                  <div className="bg-white border border-slate-100 p-6 rounded-[2rem] shadow-sm flex items-center justify-between"><span className="text-xs font-black text-slate-400 uppercase tracking-widest">{c.slides[currentSlide].v}</span><BarChart3 className="text-emerald-500" size={20}/></div>
                </div>
              </div>
            </div>
          </div>
